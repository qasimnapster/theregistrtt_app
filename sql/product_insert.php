	...
	if( $_SERVER['REQUEST_METHOD'] == 'POST' && count( $_POST ) > 0 )
	{
		if( session_id() == $_POST['token_id'] )
		{
			...
			for($i=0; $i<count($_FILES['product_image']['name']);$i++)
			{
				...
				$product_price = str_replace( '$', '', trim(str_replace('.jpg','',$exp1[2])) );
				
				$product_categ = $categorys;
	...
	<form action="#" method="POST" accept-charset="UTF-8" enctype="multipart/form-data">
		...
		<div>
			...
			<span style="font-size: 24px; display: inline-block;">1</span>
			<span id="cats-select" style="display: inline-block;">
			...
			<input type="file" name="product_image[]" accept="image/*" placeholder="Upload Product Image" style="display: inline-block; width: 75%" multiple>
		</div>
		<div id="vir"></div>
		...
		$(function(){
			...
			$('#append').on('click', function(){
				...
				for(var ix = 2; ix <=$('#numofapppend').val(); ix++)
				{
					...
					if( ix <= 20)
					{
					$('#vir').append('<div><span style="font-size: 24px;display: inline-block;">'+ix+'</span>'+ $('#cats-select').html() +'<input type="file" name="product_image[]" accept="image/*" placeholder="Upload Product Image" style="display:inline-block; width:75%"></div>');
					} else
					{
